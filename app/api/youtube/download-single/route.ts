...
interface SingleAudioDownloadRequest {
  videoId: string
}
...
export async function POST(request: NextRequest) {
  try {
    const { videoId }: SingleAudioDownloadRequest = await request.json()
    ...
    console.log(`Starting audio download and transcription for video: ${videoId}`)
    
    const result = await downloadAndTranscribeVideo(videoId)
...
export async function GET(request: NextRequest) {
  return NextResponse.json({
    message: 'Single audio download and subtitle generation API is working',
    method: 'GET',
    usage: 'Use POST method with videoId to download audio and generate subtitles for a single video using yt-dlp CLI',
