...
Type: Script Extractor Outline Generation
Filename: ${filename}
...
=== END OF PROMPT ===`;
    ...
    console.log(`📝 Script extractor outline prompt logged to: ${filepath}`);
  } catch (error) {
    console.error('❌ Failed to log script extractor outline prompt:', error);
  }
}
...
SCRIPT CONTENT: ${script.substring(0, 1000)}...
...
Do NOT include any explanation, context, or additional text.`;
    ...
    if (uploadedStyle && uploadedStyle.trim().length > 0) {
      ...
      console.log('📄 Using uploaded style guide for script extractor outline');
    } else {
      const stylePath = join(process.cwd(), 'lib', 'data', 'feeder_script_style.txt');
      styleContent = readFileSync(stylePath, 'utf-8');
      console.log('📄 Using default feeder script style for script extractor outline');
    }
...
`;
    ...
    const prompt = `You are a master storyteller and researcher creating compelling, authentic video content. Analyze the following script in detail and create a comprehensive outline for: **${title}**
...
1. Analyze the main structure and flow of the provided script
2. Identify key themes, concepts, and insights covered
3. Understand the writing style and tone used
4. Extract important points and supporting evidence
5. Determine how each part contributes to the overall message
...
1. A compelling, specific title that captures unique value from the original script
2. Comprehensive writing instructions (minimum 200 words) detailing:
   ...
   - Specific content points with supporting evidence and examples from the original script
...
Create exactly ${numSections} sections that will form a comprehensive, value-packed video script totaling ${wordCount} words, based on the structure and insights from the provided script.`;

    console.log(`🚀 Extracting script outline: "${title}" with ${numSections} sections`);

    const response = await (model as any).withStructuredOutput(scriptSectionsResponseSchema).invoke(prompt);
    
    // Log the prompt for debugging
    const sanitizedTitle = title.replace(/[^a-zA-Z0-9]/g, '_').substring(0, 50);
    logPromptToFile(prompt, `script_extractor_${sanitizedTitle}`, 'outline');

    console.log(`✅ Extracted ${response.sections.length} sections from script: "${title}"`);
