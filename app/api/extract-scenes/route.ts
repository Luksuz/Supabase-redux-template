...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!numberOfScenes || numberOfScenes < 1 || numberOfScenes > 100) {
      return NextResponse.json({ 
        error: 'Number of scenes must be between 1 and 100' 
      }, { status: 400 });
    }
    ...
    const scenePromises = limitedChunks.map(async (chunk, index) => {
      try {
        ...
        // Generate image prompt for this chunk using ChatAnthropic
        const promptResponse = await chatModel.invoke([
          {
            role: "system",
...
${chunkText}
        ...
        const promptText = promptResponse.content?.toString().trim() || 
