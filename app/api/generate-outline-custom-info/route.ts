...
CUSTOM INFORMATION: ${customInformation.substring(0, 1000)}...
...
Do NOT include any explanation, context, or additional text.`;
    ...
    if (uploadedStyle && uploadedStyle.trim().length > 0) {
      ...
      console.log('📄 Using uploaded style guide for custom info outline');
    } else {
      const stylePath = join(process.cwd(), 'lib', 'data', 'feeder_script_style.txt');
      styleContent = readFileSync(stylePath, 'utf-8');
      console.log('📄 Using default feeder script style for custom info outline');
    }
...
`;
    ...
    const prompt = `You are a master storyteller and researcher creating compelling, authentic video content. Based on the title: **${title}** and the custom information provided below, create a comprehensive script outline that transforms this content into compelling video sections.
...
1. A compelling, specific title that captures unique value from the custom information
2. Comprehensive writing instructions (minimum 200 words) detailing:
   ...
   - Specific content points with supporting evidence and examples from the source material
   ...
   - Concrete examples, analogies, or case studies to include from the custom information
   - Key questions to address or insights to reveal
   - Emotional pacing and tonal shifts throughout the section
   - Specific facts, statistics, or expert perspectives to reference from the source
...
Create exactly ${numSections} sections that will form a comprehensive, value-packed video script totaling ${wordCount} words, based on the provided custom information.`;

    console.log(`🚀 Generating custom information outline: "${title}" with ${numSections} sections`);
    ...
    console.log(`✅ Generated ${response.sections.length} sections from custom information: "${title}"`);
