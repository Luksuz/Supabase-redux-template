...
export async function GET(request: NextRequest) {
  console.log('=== GET /api/fine-tuning/export-sections-training ===')
  ...
  try {
    ...
    try {
      const { trainingData } = await getSectionDataSplit(user.id, 3)
      ...
      console.log(`Returning ${trainingData.length} training examples`)
      ...
      return NextResponse.json({ error: 'Failed to process training data' }, { status: 500 })
    }

  } catch (error) {
    console.error('Unexpected error in export-sections-training GET:', error)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
}

export async function POST(request: NextRequest) {
  console.log('=== POST /api/fine-tuning/export-sections-training ===')
  ...
  try {
    ...
    try {
      const { trainingData, summary } = await getSectionDataSplit(user.id, minSections)
      ...
      console.log(`Generated section training examples: ${summary.trainingExamples}/${summary.totalExamples}`)
    ...
    console.error('Unexpected error in export-sections-training POST:', error)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
} 